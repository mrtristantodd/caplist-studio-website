...
- Public GitHub repository: https://github.com/mrtristantodd/caplist-studio-website
...
The repository was made public at the owner's explicit request. GitHub main-branch protection is enabled: force pushes and deletion are disallowed, and the rule applies to administrators. No mandatory PR reviews or status checks are required. The approved tag and backup branch remain fixed recovery references by convention; they do not have separate server-enforced protection.
...
Public recovery does not require GitHub authentication. Push access still requires an authorized account. The hash above must match the exact approved baseline. Earlier “no remote” statements describe the initial local snapshot, before this separate remote-recovery step.
